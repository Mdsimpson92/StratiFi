...
export default function Dashboard() {
  ...
  function buildSnapshot(): SnapshotData {
    ...
    return {
      safe:    safeRec ? safeRec.suggested_action : null,
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/demo-status').then(r         => r.json()).catch(() => ({ is_demo: false })),
    ])
      .then(([ins, cash, pat, ano, spend, plaid, subs, fc, alrt, recs, sc, alloc, demo]) => {
        setIsDemo(demo?.is_demo ?? false)
  ...
  return (
    <main style={styles.page} className="pwa-page">
      ...
      <section style={styles.section} className="pwa-section">
        ...
        <HelpToggle id="allocation" open={!!helpOpen.allocation} onToggle={toggleHelp} text="See how your money is split between emergency fund, retirement, and debt. The bars show where you stand versus your targets — green means on track, orange means behind." />
      ...
      {alerts.filter(a => !a.dismissed).length > 0 && (!isMobile || activeTab === 'alerts') && (
        <section style={styles.section} className="pwa-section">
          ...
          <HelpToggle id="alerts" open={!!helpOpen.alerts} onToggle={toggleHelp} text="We watch your transactions for unusual activity — spending spikes, missed bills, surprise charges — and surface them here. Enable push to get notified the moment something happens." />
          {/* iOS PWA guide — push requires Home Screen installation on iPhone */}
          {notifPermission !== 'granted' && isIOS && !isPWA && (
      ...
      <section style={styles.section} className="pwa-section">
        <h2 style={styles.heading}>Top Actions</h2>
        <p style={SOURCE_NOTE}>
          Ranked by impact. Based on your spending, bills, and financial profile.
        </p>
        <HelpToggle id="actions" open={!!helpOpen.actions} onToggle={toggleHelp} text="The highest-leverage moves you can make right now, ranked by impact. Each one tells you the specific action and the dollar amount it could save or unlock." />
      ...
      {/* ── Forecast ─────────────────────────────────────────────────────── */}
      <section style={styles.section} className="pwa-section">
        <h2 style={styles.heading}>30-Day Forecast</h2>
        <p style={SOURCE_NOTE}>
          Projected from your recurring charges and income patterns. Assumes bills continue at their current amount and frequency.
        </p>
        <HelpToggle id="forecast" open={!!helpOpen.forecast} onToggle={toggleHelp} text="Projects your next 30 days of income and bills based on recurring patterns we've detected. Useful for spotting cash crunches before they happen." />
        {!forecast ? (
            ...
            <div style={styles.pieWrapper} className="pwa-pie-wrapper">
              ...
              <div style={styles.pieDetail}>
                {selectedCategory ? (
                  ...
                  <p style={styles.empty}>Click a slice to see details.</p>
                )}
              </div>
            </div>
          )
...
// ─── HelpToggle ──────────────────────────────────────────────────────────────
// Permanent inline "What is this?" expander so users can learn what each
// section does at their own pace, even after dismissing the guided tour.

function HelpToggle({ id, open, onToggle, text }: {
  id: string
  open: boolean
  ...
  return (
    <div style={HELP_WRAP}>
      <button type="button" style={HELP_BTN} onClick={() => onToggle(id)} aria-expanded={open}>
        <span style={HELP_ICON}>{open ? '▾' : '▸'}</span>
        <span>What is this?</span>
      </button>
      {open && <p style={HELP_BODY}>{text}</p>}
    </div>
  )
}

const HELP_WRAP: React.CSSProperties = { margin: '0 0 0.75rem' }
const HELP_BTN: React.CSSProperties = {
  background: 'none', border: 'none', padding: '0.25rem 0',
  fontSize: '0.75rem', color: '#2ab9b0', fontWeight: 600,
  cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.3rem',
}
const HELP_ICON: React.CSSProperties = { fontSize: '0.7rem', color: '#2ab9b0' }
const HELP_BODY: React.CSSProperties = {
  margin: '0.4rem 0 0', fontSize: '0.82rem', color: '#5b7a99',
  lineHeight: 1.55, background: '#f0f9fb', border: '1px solid #cce6ea',
